...
if not st.session_state.init:
    ...
    if st.button("התחל"):
        if txt:
            res = client.chat.completions.create(
                model="gpt-4o",
                messages=[{"role": "system", "content": "You are a sharp Profiler. Respond in Hebrew, 1st person, very brief. One sentence + one DNA question."}] + [{"role": "user", "content": txt}]
            )
    ...
    if p := st.chat_input("שתף מחשבה או משימה..."):
        ...
        prompt = "Sharp Profiler. Hebrew, 1st person. Brief insight + question. Return JSON: {'res': 'text', 'stats': {}, 'conf': int, 'ins': 'text'}"
        
        raw = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": prompt}] + st.session_state.messages[-10:],
            response_format={"type": "json_object"}
        )
        
        try:
            data = json.loads(raw.choices[0].message.content)
            st.session_state.messages.append({"role": "assistant", "content": data.get('res', 'המשך..')})
            ...
            st.session_state.messages.append({"role": "assistant", "content": "הבנתי. מה הצעד הבא?"})
        st.rerun()
